...
const clean=(v,m=4000)=>String(v??'').replace(/[<>]/g,'').trim().slice(0,m);
module.exports=async(req,res)=>{
 if(req.method==='GET'){
  const keys=['SMTP_HOST','SMTP_PORT','SMTP_SECURE','SMTP_USER','SMTP_PASS','CONTACT_EMAIL'];
  return res.status(200).json({ok:true,service:'OrdenadoresMoncloa contacto API',environment:Object.fromEntries(keys.map(k=>[k,Boolean(process.env[k])]))});
 }
 ...
 try{
  ...
  await transporter.sendMail({from:`"OrdenadoresMoncloa" <${process.env.SMTP_USER}>`,to:process.env.CONTACT_EMAIL||process.env.SMTP_USER,replyTo:email,subject:'Nueva consulta OrdenadoresMoncloa - asusplace.com.es',text:`Nombre: ${nombre}\nTeléfono: ${telefono}\nEmail: ${email}\nEquipo: ${equipo}\n\nConsulta:\n${mensaje}`});
  return res.status(200).json({ok:true});
 }catch(error){console.error(error);return res.status(500).json({ok:false,code:'SMTP_SEND_FAILED',detail:error?.code||'UNKNOWN'});}
};
