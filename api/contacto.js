...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    return res.status(200).json({
      ok: true,
      service: 'OrdenadoresMoncloa contacto API',
  ...
  try {
    ...
    if (missingEnv.length) {
      console.error('OrdenadoresMoncloa: faltan variables', missingEnv);
    ...
    const subject = 'Nueva consulta OrdenadoresMoncloa - asusplace.com.es';

    const body = `
      <h2>Nueva consulta OrdenadoresMoncloa</h2>
      <p><strong>Web:</strong> asusplace.com.es</p>
      <p><strong>Nombre:</strong> ${n}</p>
      <p><strong>Teléfono:</strong> ${t}</p>
      <p><strong>Email:</strong> ${e}</p>
      <p><strong>Equipo / modelo:</strong> ${mo || 'No indicado'}</p>
      <p><strong>Avería:</strong><br>${msg.replace(/\n/g, '<br>')}</p>
    `;

    const raw = [
      `From: OrdenadoresMoncloa <${process.env.GOOGLE_EMAIL}>`,
    ...
    console.error('OrdenadoresMoncloa Gmail API:', {
      message,
    ...
    if (responseError === 'invalid_grant' || message.includes('invalid_grant')) {
      code = 'GOOGLE_OAUTH_INVALID_GRANT';
    } else if (responseError === 'invalid_client' || message.includes('invalid_client')) {
